...
public class Commands implements CommandExecutor, TabCompleter {
    ...
    public boolean onCommand(@NotNull CommandSender commandSender, @NotNull Command command, @NotNull String s, @NotNull String[] strings) {
        ...
        if (commandSender instanceof Player player){
            ...
            switch (subCommand){
                case "set" -> {
                    if (player.hasPermission("nick.commands.set")){
                        if (strings.length != 2){
                            player.sendMessage(miniMessage.deserialize("<red>usage: /nick set <nickname>"));
                            return true;
                        }
                ...
                case "setother" -> {
                    if (player.hasPermission("nick.commands.setother")){
                        if (strings.length != 3){
                            player.sendMessage(miniMessage.deserialize("<red>usage: /nick setother <player> <nickname>"));
                            return true;
                        }
                ...
                case "unset" -> {
                    if (player.hasPermission("nick.commands.unset")){
                        final Scoreboard scoreboard = Bukkit.getScoreboardManager().getMainScoreboard();
                        final Team team = scoreboard.getPlayerTeam(player);
                        if (team == null){
                ...
                case "unsetother" -> {
                    if (player.hasPermission("nick.commands.unsetother")){
                        if (strings.length != 2){
                            player.sendMessage(miniMessage.deserialize("<red>usage: /nick unsetother <player>"));
                            return true;
                        }
